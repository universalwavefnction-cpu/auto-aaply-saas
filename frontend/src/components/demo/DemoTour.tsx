...
import DemoBotLive from './DemoBotLive'
import DemoApplications from './DemoApplications'
import DemoSignup from './DemoSignup'

const stepIcons = [LayoutDashboard, UserCircle, Bot, Send, Rocket]
...
function TourOverlay({
  ...
  if (step === 4) return null // Sign up page has its own CTA
    ...
    <>
      ...
      <div className="fixed top-[53px] left-0 right-0 z-50 md:hidden px-3 pt-2">
        <div className="rounded-xl border border-white/10 bg-[#0A0A0A]/95 backdrop-blur-xl px-3.5 py-3 shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
          {/* Row 1: icon + title + step counter + next button */}
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={onNext}
                className="flex items-center gap-1 rounded-lg bg-amber-500 px-3.5 py-1.5 text-[9px] font-black uppercase tracking-wider text-black"
              >
                {isLast ? 'Finish' : 'Next'}
      ...
      <div className="hidden md:block fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-full max-w-md px-4">
        <div className="rounded-2xl border border-white/10 bg-[#0A0A0A]/95 backdrop-blur-xl p-6 shadow-[0_0_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onNext}
              className="group flex items-center gap-1.5 rounded-lg bg-amber-500 px-4 py-2 text-[10px] font-black uppercase tracking-wider text-black transition-all hover:bg-amber-400"
            >
              {isLast ? 'Finish' : 'Next'}
...
export default function DemoTour() {
  ...
  const { step, direction, next, back, goTo, isFirst, total } = useDemoTour()

  const handleSkip = () => navigate('/login?register=1')
  const handleNext = () => {
    if (step === 4) {
  ...
  const stepComponents = [
    ...
    <DemoProfile key="profile" />,
    <DemoBotLive key="bot" />,
    <DemoApplications key="applications" />,
    <DemoSignup key="signup" />,
  ]

  return (
    <div className="flex h-screen bg-[#050505] font-sans text-white overflow-hidden selection:bg-amber-500/30">
      {/* Desktop Sidebar */}
      <DemoSidebar step={step} onGoTo={goTo} onSkip={handleSkip} />
      ...
      <div className="fixed top-0 left-0 right-0 z-40 flex md:hidden items-center justify-between border-b border-white/5 bg-[#080808]/95 backdrop-blur-xl px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="rounded-md border border-amber-500/20 bg-amber-500/5 px-2 py-0.5 text-[8px] font-bold uppercase tracking-widest text-amber-500">
            Demo
          </span>
        </div>
        <button
          onClick={handleSkip}
          className="flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-black"
        >
          Sign Up
          <ArrowRight className="h-3 w-3" />
        </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 flex md:hidden border-t border-white/5 bg-[#080808]/95 backdrop-blur-xl">
        {[
          { s: 0 as DemoStep, icon: LayoutDashboard, label: 'Home' },
          { s: 1 as DemoStep, icon: UserCircle, label: 'Profile' },
          { s: 2 as DemoStep, icon: Bot, label: 'Bot' },
          { s: 3 as DemoStep, icon: Send, label: 'Apps' },
          { s: 4 as DemoStep, icon: Rocket, label: 'Sign Up' },
